  ...
  *right = self->width - x - width;
  ...
  if (width_ratio > 0 && height_ratio > 0) {
    gint top_bottom_modify =
        (gint) round (((height_ratio * width) / width_ratio - height) / 2);
    gint left_right_modify =
        (gint) round (((width_ratio * height) / height_ratio - width) / 2);
...
gst_inference_crop_find_predictions (GstInferenceCrop *self,
  ...
  if (false == static_cast<bool> (G_NODE_IS_ROOT (pred->predictions)) && true == static_cast<bool> (pred->enabled)) {
